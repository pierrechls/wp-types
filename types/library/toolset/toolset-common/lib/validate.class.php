...
if ( ! class_exists( 'Toolset_Validate' ) ) {
	    ...
	    public static function check($args, $value)
	    {
	        ...
	        foreach ($args as $method => $v) {
	            ...
	            if (is_callable(array('Toolset_Validate', $method))) {
	                $check = call_user_func_array(array('Toolset_Validate', $method),
	                        array($v, $value));
	                // Use CakePHP method
	            } else if ((isset(self::$_cake_aliases[$method])
	                    && is_callable(array('Toolset_CakePHP_Validation', self::$_cake_aliases[$method])))
	                    || is_callable(array('Toolset_CakePHP_Validation', $method))) {
	...
	//                                    array_values($v));
	    ...
	    {
	        return (is_callable(array('Toolset_Validate',  $method))
	    ...
	    {
	        return is_callable(array('Toolset_Validate', $method . '_form'));
	    }
